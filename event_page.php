<?php
	...
	if (mysqli_num_rows($query)>0 || check_permission($connection, "delete_event")){
		...
		$string_query_reviews = "SELECT * FROM tbl_reviews INNER JOIN tbl_users ON tbl_reviews.id_user = tbl_users.id_user WHERE id_event='$id_event'";
		if(!$query_event_reviews = mysqli_query($connection, $string_query_reviews)){
			$smarty->assign('message','ERROR: SQL error, try again! '.mysqli_error($connection));
			$smarty->display('message.tpl');
			mysqli_close($connection);
			die();
		...
		if (mysqli_num_rows($query_event_reviews)>0){
			while ($row = mysqli_fetch_assoc($query_event_reviews)){
				$event_reviews[] = $row;
			}
		}
	}
	...
	$p = new Pagination($connection, $string_query_reviews, $x_pagina, "pagina");

	if($record_s = $p->Show()){
	$smarty->assign("event_reviews",$record_s);
	/*passo il numero di righe per decidere se inserire o no la scelta del numero di prodotti per pagina*/
	$smarty->assign("num_tot",mysqli_num_rows($query_event_reviews));
	}
