      ...
      <div class="container">
        <div class="row">
          ...
          {/if}
            ...
            <h3 align="center">Participants</h3><br>
                          ...
                          <div class="friend-card">
                            ...
                            <div class="card-info">
                              ...
                              <div class="friend-info">
                                ...
                                <!-- About me maybe -->
                                <p>{$smarty_participants.type_participation}</p>
                              </div>
                            </div>
                          </div>
                        </div>
          ...
          {foreach name=tbl_events item=smarty_event_information from=$event_information}
            <div class="post-pagination">
              <div class="pagination">
                {if isset($_prima)}
                  <a href="event_participants.php?id_event={$smarty_event_information.id_event}&pagina={$_prima}" class="links ion-chevron-left"></a>
                {/if}
                {if isset($_prec)}{foreach item=precedente from=$_prec}
                  <a href="event_participants.php?id_event={$smarty_event_information.id_event}&pagina={$precedente}" class="links">{$precedente}</a>
                {/foreach}{/if}
                {if isset($_corr)}
                  <a>{$_corr}</a>{/if}
                  {if isset($_succ)}{foreach item=successivo from=$_succ}
                    <a href="event_participants.php?id_event={$smarty_event_information.id_event}&pagina={$successivo}" class="links">{$successivo}</a>
                  {/foreach}{/if}
                {if isset($_ultima)}
                  <a href="event_participants.php?id_event={$smarty_event_information.id_event}&pagina={$_ultima}" class="links ion-chevron-right"></a>
                {/if}
              </div>
            </div>
          {/foreach}
