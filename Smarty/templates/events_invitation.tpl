      ...
      <div class="container">
        <div class="row">
          ...
          <div class="col-md-3 static">
            ...
            <!-- Post Content
            ================================================= -->
            <h3 align="center">Events Invitation</h3><br>
                  ...
                  <div class="row">
                    {if isset($my_events)}
                      {foreach name=tbl_events item=smarty_my_events from=$my_events}
                        {if $smarty_my_events.yes_or_not != yes}
                            ...
                            <div class="friend-card">
                              ...
                              <div class="card-info">
                                ...
                                <div class="friend-info">
                                  ...
                                  <!-- About me maybe -->
                                  <p style="float: right;"><a href="accept_event.php?id_event={$smarty_my_events.id_event}"><button class="btn btn-make">Accept</button></a></p>
                                </div>
                              </div>
                            </div>
                          </div>
            ...
            <div class="post-pagination">
              <div class="pagination">
                {if isset($_prima)}
                  <a href="events_invitation.php?pagina={$_prima}" class="links ion-chevron-left"></a>
                {/if}
                {if isset($_prec)}{foreach item=precedente from=$_prec}
                  <a href="events_invitation.php?pagina={$precedente}" class="links">{$precedente}</a>
                {/foreach}{/if}
                {if isset($_corr)}
                  <a>{$_corr}</a>{/if}
                  {if isset($_succ)}{foreach item=successivo from=$_succ}
                    <a href="events_invitation.php?pagina={$successivo}" class="links">{$successivo}</a>
                  {/foreach}{/if}
                {if isset($_ultima)}
                  <a href="events_invitation.php?pagina={$_ultima}" class="links ion-chevron-right"></a>
                {/if}
            </div>
          </div>
        </div>
